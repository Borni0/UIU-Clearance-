...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($id === 'all') {
            ...
            $s->execute([(int)$me['id']]);
            flash_ok('All notifications marked as read.');
        } else {
            $nid = (int)$id;
            if ($nid > 0) {
    ...
    header('Location: ' . ($_POST['back'] ?? '/student/notifications.php'));
    exit;
}
...
<div class="layout">
    ...
    <main class="main">
        <div class="page-head">
            <h1>Notifications</h1>
            <p>Updates from admins, the VC, and the system.</p>
            <?php if (!empty($rows)): ?>
                <div class="actions">
                    <form method="POST" style="display:inline;">
                        <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
                        <input type="hidden" name="id" value="all">
                        <input type="hidden" name="back" value="/student/notifications.php">
                        <button class="btn btn-secondary btn-sm" type="submit"><i class="fa-solid fa-check-double"></i> Mark all as read</button>
                    </form>
                </div>
            <?php endif; ?>
        </div>
